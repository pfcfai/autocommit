...
#Sql select 資料庫查詢語法
x=40
engine = create_engine('mysql+pymysql://root:pfcfai@localhost:3306/stockfuturedata_day')
sqllist = 'select futuresymbol from stockfuturedata'
symbollist = pd.read_sql_query(sqllist, engine).drop_duplicates()
...
for i in trange(len(symbollist)):
    ...
    sql = "select Date,Open,High,Low,Close from stockfuturedata where futuresymbol = '"+stocksymbol+"' order by Date asc"
...
#        print(pp[0])
    ...
    #        #plt.ylabel("Price")
            ...
            fig.savefig("C:\\python_code\\KBarRecognition\\ModelSelect\\StockfutureDayLong\\"+stocksymbol+".jpg",dpi=300)
            plt.close(fig)
        #分類空
        elif pp[0][2]==1:
            sql = "select Date,Open,High,Low,Close from stockfuturedata where futuresymbol = '"+stocksymbol+"' order by Date asc"
    ...
    #        #plt.ylabel("Price")
            ...
            plt.yticks([])
            candlestick_ohlc(ax, tuples, width=.6,colordown='g', colorup='r', alpha =.4)
            fig.savefig("C:\\python_code\\KBarRecognition\\ModelSelect\\StockfutureDayShort\\"+stocksymbol+".jpg",dpi=300)       
            plt.close(fig)
