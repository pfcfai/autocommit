...
# 以下是開發時常用的小技巧
# 包含 
#  * datetime 轉換 字串或 linuxTimestamp
#  * list 時序減法
#  * Pandas read csv
#  * openpyxl read excel whole sheet 寫法
...
from datetime import timezone
from datetime import datetime
print("datetime to linuxTime:",date[-1].replace(tzinfo=timezone.utc).timestamp()*1000)
#date_time = date[-1].strftime("%m/%d/%Y, %H:%M:%S")
date_time = date[-1].strftime("%m/%y")
print("datetime to str:",date_time)	
...
# list 時序減法
sr=[i for i in range(1,11)]
print(sr)
ar=[sr[j]-sr[j-1] for j in range(len(sr)) if j>0]
print(ar)
